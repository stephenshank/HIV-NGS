...
def pairwise_distance_estimator (node_to_run_on):
    ...
    while True:
        ...
        try:
            command = ['tn93', '-t', str(threshold), '-l', '200', '-u', str(subsample), '-c', '-q', '-s', in_path2, in_path1]
            print('RUNNING:', ' '.join(command))
            process = subprocess.Popen (command, stdin = subprocess.DEVNULL, stderr = subprocess.PIPE, stdout = subprocess.PIPE, universal_newlines = True)
