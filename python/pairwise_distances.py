...
def pairwise_distance_estimator (node_to_run_on):
    ...
    while True:
        ...
        try:
            process = subprocess.Popen (['tn93', '-t', str(threshold), '-l', '200', '-u', str(subsample), '-c', '-q', '-s', in_path2, in_path1], stdin = subprocess.DEVNULL, stderr = subprocess.PIPE, stdout = subprocess.PIPE, universal_newlines = True)
